...
"""Functionality to encode and decode miniscripts.
"""
from itertools import product

from .key import ECPubKey
from .script import CScript, OP_VERIFY, OP_ADD, OP_BOOLOR, OP_DUP, \
    OP_HASH160, OP_EQUAL, OP_EQUALVERIFY, OP_CHECKSIG, OP_SIZE, OP_SWAP, \
    OP_IFDUP, OP_NOTIF, OP_ENDIF, OP_IF, OP_ELSE, \
    OP_CHECKSEQUENCEVERIFY, OP_CHECKLOCKTIMEVERIFY, OP_BOOLAND, \
    OP_TOALTSTACK, OP_FROMALTSTACK, OP_CHECKMULTISIG, OP_CHECKMULTISIGVERIFY, \
    OP_CHECKSIGVERIFY, OP_0NOTEQUAL, OP_SHA256, OP_RIPEMD160, OP_HASH256
...
class SatType(Enum):
    ...
    # The value provides a hint how to construct the respective witness element.
    OLDER = 0                       # Value: Delay
    AFTER = 1                       # Value: Time
...
class Node:
    ...
    def _generate_child_sat_dsat_ncan(self):
        ...
        # Iterators must be list iterators to be reusable.
    ...
    @staticmethod
    def from_script(expr_list_in):
        expr_list = expr_list_in.copy()
    ...
    def _parse_expr_list(expr_list):
        ...
        while idx >= 0:
            ...
            if expr_list_len-idx >= 5 and \
                ...
                k = 1 if isinstance(expr_list[idx], Node) else expr_list[idx]
                ...
                for n in range(k, expr_list_len-2):
                    ...
                    if match is True and \
                            ((
                            isinstance(expr_list[idx+n+1], int) and
                            expr_list[idx+n+1] == len(pk_m)
                            ) or (
                            isinstance(expr_list[idx+n+1], Node) and
                            expr_list[idx+n+1].t == NodeType.JUST_1
                            )) and \
            ...
            if expr_list_len-idx >= 7:
                ...
                for n in range(3, int((expr_list_len-1)/2)+1):
                    ...
                    if match is True and \
                            isinstance(expr_list[idx], Node) and \
                            isinstance(expr_list[thresh_expr_len-2], int) and \
    ...
    def from_desc(string):
        ...
        # Alternative node constructors forms:

        # Alternative terminal node constructor.
        if tag == 'thresh_m':
        ...
        # Alternative non-terminal node constructor.
        if tag == 'thresh':
            k = int(child_exprs.pop(0))
    ...
    def construct_and_b(self, child_x, child_y):
        ...
        self._construct(NodeType.AND_B, Property().from_string(prop_str),
                        ...
                        child_x.script+child_y.script+[OP_BOOLAND],
    ...
    def construct_and_n(self, child_x, child_y):
        ...
        self._construct(NodeType.AND_N, Property().from_string(prop_str),
                        ...
                        child_x.script+[OP_NOTIF, 0, OP_ELSE]+child_y.script+[
                            OP_ENDIF],
    ...
    def construct_or_b(self, child_x, child_z):
        ...
        self._construct(NodeType.OR_B, Property().from_string(prop_str),
                        ...
                        child_x.script+child_z.script+[OP_BOOLOR],
    ...
    def construct_or_d(self, child_x, child_z):
        ...
        self._construct(NodeType.OR_D, Property().from_string(prop_str),
                        ...
                        child_x.script+[OP_IFDUP, OP_NOTIF]+child_z.script+[
                            OP_ENDIF],
    ...
    def construct_or_c(self, child_x, child_z):
        ...
        self._construct(NodeType.OR_C, Property().from_string(prop_str),
                        ...
                        child_x.script+[OP_NOTIF]+child_z.script+[OP_ENDIF],
    ...
    def construct_or_i(self, child_x, child_z):
        ...
        self._construct(NodeType.OR_I, Property().from_string(prop_str),
                        ...
                        [OP_IF]+child_x.script+[OP_ELSE]+child_z.script+[
                        OP_ENDIF],
    ...
    def construct_andor(self, child_x, child_y, child_z):
        ...
        self._construct(NodeType.ANDOR, Property().from_string(prop_str),
                        ...
                        child_x.script+[OP_NOTIF]+child_z.script+[
                        OP_ELSE]+child_y.script+[OP_ENDIF],
    ...
    def construct_a(self, child_x):
        ...
        self._construct(NodeType.WRAP_A, Property().from_string(prop_str),
                        ...
                        [OP_TOALTSTACK]+child_x.script+[OP_FROMALTSTACK],
                        tag+child_x.desc
                        )
        return self
    ...
    def construct_s(self, child_x):
        ...
        self._construct(NodeType.WRAP_S, Property().from_string(prop_str),
                        ...
                        [OP_SWAP]+child_x.script, tag+child_x.desc
                        )
        return self
    ...
    def construct_c(self, child_x):
        ...
        self._construct(NodeType.WRAP_C, Property().from_string(prop_str),
                        ...
                        child_x.script+[OP_CHECKSIG], tag+child_x.desc
                        )
        return self
    ...
    def construct_t(self, child_x):
        ...
        self._construct(NodeType.WRAP_T, Property().from_string(prop_str),
                        ...
                        child_x.script+[1], tag+child_x.desc
                        )
        return self
    ...
    def construct_d(self, child_x):
        ...
        self._construct(NodeType.WRAP_D, Property().from_string(prop_str),
                        ...
                        [OP_DUP, OP_IF]+child_x.script+[OP_ENDIF],
                        tag+child_x.desc
                        )
        return self
    ...
    def construct_v(self, child_x):
        ...
        if child_x.script[-1] == OP_CHECKSIG:
            script = child_x.script[:-1]+[OP_CHECKSIGVERIFY]
        elif child_x.script[-1] == OP_CHECKMULTISIG:
            script = child_x.script[:-1]+[OP_CHECKMULTISIGVERIFY]
        elif child_x.script[-1] == OP_EQUAL:
            script = child_x.script[:-1]+[OP_EQUALVERIFY]
        else:
            script = child_x.script+[OP_VERIFY]
    ...
    def construct_j(self, child_x):
        ...
        script = [OP_SIZE, OP_0NOTEQUAL, OP_IF]+child_x.script+[OP_ENDIF]
    ...
    def construct_n(self, child_x):
        ...
        self._construct(NodeType.WRAP_N, Property().from_string(prop_str),
                        ...
                        child_x.script+[OP_0NOTEQUAL], tag+child_x.desc)
        return self
    ...
    def construct_l(self, child_x):
        ...
        script = [OP_IF, 0, OP_ELSE]+child_x.script+[OP_ENDIF]
    ...
    def construct_u(self, child_x):
        ...
        script = [OP_IF]+child_x.script+[OP_ELSE, 0, OP_ENDIF]
    ...
    def construct_thresh(self, k, children_n):
        ...
        if child_is_s_count >= (n-k+1):
            prop_str += "s"
        script = []
        script.extend(children_n[0].script)
        for child in children_n[1:]:
            script.extend(child.script+[OP_ADD])
    ...
    def _construct(self,
        ...
        # User calls .sat_ncan wrapper method, which handles [[None]]
    ...
    # Utility methods.
    @staticmethod
    def _is_wrapper(child):
        if child.t in [NodeType.WRAP_A,
                       NodeType.WRAP_C,
                       NodeType.WRAP_D,
                       NodeType.WRAP_J,
                       NodeType.WRAP_N,
